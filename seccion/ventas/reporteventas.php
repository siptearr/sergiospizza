...
$idmultipleservicios = $_SESSION["idmultipleservicios"];
$idmultiplecauses = $_SESSION["idmultiplecauses"];
...
if($filtro != ""){
    $querycontruido .= 'where a.afiliado_idafiliado = '.$filtro.' and a.estatus<>\'inactivo\' ';
    if($idmultipleservicios != "" && $idmultiplecauses != ""){
        $querycontruido .= 'and a.multipleservicios_idmultipleservicios='.$idmultipleservicios.' ';
        $querycontruido .= 'and a.multiplecauses_idmultiplecauses='.$idmultiplecauses.' ';
    }
    $querycontruido .= 'group by a.id'.$catalogo.' ';
}
$querycontruido .= 'order by a.id'.$catalogo.' desc';
...
<table cellspacing="0" class="ewTable ewTableSeparate" id="Tcolor">
	<thead>
		<tr>';
for($i=1;$i<$numcamps;$i++){
    if($encabezados[$i] == "estatus"){
        $columnaestatus = 0;
    }else{
...
<tr class="ewTableRow" >
_HTML_;
    for($a=1;$a<$numcamps;$a++){
        if(utf8_encode($arregloresultado[$base][$a]) == "activo"){
            if($columnaestatus == 1){
 /*
 * Icono de reporte de servicios y causes
 */
                $w++;
                switch($w){
                    case "1":
                        $funcion = "muestraservicios(";
                    break;
                    case "2":
                        $funcion = "muestracauses(";
                    break;
                default:
                }
                $tabla.=<<<_HTML_
...
_HTML_;
                $tabla.= $funcion;
                $tabla.= $arregloresultado[$base][0];
                $tabla.=<<<_HTML_
);" ><img src="imagenes/logopdf.jpg" height="20" width="15" /></a>
_HTML_;
/*
 * Icono de reporte servicios y causes
 */               
            }else{
                $columnaestatus = 1;  
            }
...
_HTML_;
    $tabla.= $arregloresultado[$base][0];
    $tabla.=<<<_HTML_
);" ><img src="imagenes/eliminafila.jpg" height="20" width="15" /></a>
_HTML_;
/*
 * Icono de eliminar fila
 */
