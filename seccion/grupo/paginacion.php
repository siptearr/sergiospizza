...
class PHPPaging {
	...
	*	@access private
	*/
	function asignacatalogo($catalogo) {
	...
	function fetchNavegacion () {
		...
		if(empty($this->linkEstructura)) {
			...
			//$this->linkEstructura = 'http://'.trim($_SERVER['HTTP_HOST'], '/').'/'.ltrim($_SERVER['PHP_SELF'], '/').'?'.$this->query_string;
                        $num_pagina = $this->query_string;
                        $numpagina=explode("=",$num_pagina);
                        $num_pagina = $numpagina[2];
                        $catalogo  = $this->catalogo;
                        $this->linkEstructura = "javascript:muestrareporte(".$num_pagina.",'".$catalogo."')";
			//$this->linkEstructura = "xajax_LanzaVentana($this->query_string);";
	...
	function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
		if($this->modo != 'publicacion' and $this->mostre_error == false) {
			if($this->modo == 'reporte') {
				...
				$error.= "Si no logra solucionar el problema, env�e un mensaje a Admin indicando el error mostrado y la consulta que gener� el error";
			} else {
				$error = "Hubo un error ejecutando la paginaci�n. El mensaje devuelto es: <blockquote><code style='color: #080; font-size: 13px;'><strong>$msg</strong></code></blockquote>Si no logra solucionar el problema, env�e un mensaje admin indicando el error mostrado y la consulta que gener� el error";
			}
			$p = "<div style='border: 1px solid #666; background-color: #f6f6f6; margin: 5px 10px; padding: 10px 5px;'>";
			$p.= "<div style='color: #f00; font: 18px Arial; margin: 0 5px; border-bottom: 1px dotted #f00;'><strong>Error</strong></div>";
			$p.= "<div style='font: 12px Arial; margin: 15px 19px;'>$error</div>";
			$p.= "<div style='font: 12px Arial; text-align: right; color: #777; margin: 0; padding: 1px 4px;'></div></div>";
			echo $p;
		} else {
			$this->error = $msg;
		}
