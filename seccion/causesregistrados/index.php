...
$catalogoaminar1 = 'causesregistrados';
...
$contenidoinserta = '<?php
    ...
    if($rs1){
        echo \'causesregistrados*correcto*\'.mysql_insert_id();
    }else{
        echo \'causesregistrados*incorrecto\';
    }
    mysql_close($conn);
?>';
...
$contenidoelimina = '<?php
    ...
    if($row2["resultadoconteo"]==0){
        echo \'causesregistrados*correcto*\'.$row2["resultadoconteo"];
    }else{
        echo \'causesregistrados*correcto*\'.$idregistro;
    }
    mysql_close($conn2);
?>';
//Script para eliminar
$contenidoeliminacompuesta = '<?php
    ...
    $rs1 = mysql_query($sql1,$conn);
    if($rs1){
        echo "causesregistrados*correcto*".$idregistro1."-".$idregistro2;
    }else{
        echo "causesregistrados*incorrecto";
    }
    mysql_close($conn);
...
echo '<b><span class="Estilo2" style="font-family:&quot;Arial&quot;,&quot;sans-serif&quot;">Captura de Causes</span></b>
<hr>';
 ...
 */
                        ...
                        case 'causesregistrados':
                            ...
                            case "causes_idcauses":
                                ...
                                while($registrocompuesto=mysql_fetch_array($rscompuesto)){
                                    $banderamodal=1;
                                    echo '<option value="'.$registrocompuesto[0].'">';
                                        if(substr(utf8_encode($registrocompuesto[2]),51,1)!=""){
                                            $puntos= "...";
                                        }else{
                                            $puntos= "";
                                        }
                                        echo utf8_encode($registrocompuesto[1])."-".substr(utf8_encode($registrocompuesto[2]),0,50).$puntos;
                                    echo '</option>';
                                }
                                echo '</select>';
                            break;
                    ...
                    echo '<td colspan="2">Agrega cada uno de los '.substr(ucfirst(substr($registro2["Field"],2)),0,$longcatmult).' brindados en la Atencion</td>';
                }
            }
            echo '</tr>';
        }
 ...
 */
        ...
        echo '<input type="submit" id="botonguardar" name="botonguardar" value="Agrega Causes"/>
