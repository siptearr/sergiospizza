...
$longcatalogo = strlen($catalogo)-11;
$subcatalogo = substr($catalogo,0,$longcatalogo);
if($filtro != ""){
    $querycontruido .= 'where a.multiple'.$subcatalogo.'_idmultiple'.$subcatalogo.' = '.$filtro.' and a.estatus<>\'inactivo\' ';
    $querycontruido .= 'group by a.'.$subcatalogo.'_id'.$subcatalogo.' ';
}
...
for($i=1;$i<$numcamps;$i++){
    if($encabezados[$i] != "estatus"){
        if($encabezados[$i] != "multipleservicios"){
...
_HTML_;
    for($a=1;$a<$numcamps;$a++){
        if(utf8_encode($arregloresultado[$base][$a]) != "activo"){
            if(utf8_encode($arregloresultado[$base][$a]) != "inactivo"){
                $tabla.=<<<_HTML_
